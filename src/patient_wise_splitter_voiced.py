...
dataset_path = Path("..", "datasets", "patients_wise_split_voiced")
...
patients_ids = [filename.name.lstrip("voice")[:3] for filename in path_to_dataset.glob("*.png")]
...
for id in tqdm(patients_ids):
    ...
    for segment in path_to_dataset.glob(f"voice{id}*.png"):
